...
public class PrepaidSaleForm extends XJDialog {
	...
	private XJLabel lblNominal;
	private XJComboBox cmbNominal;
	...
	public PrepaidSaleForm(Window parent) {
		...
		lblNominal = new XJLabel();
		lblNominal.setText("Nominal (Rp)");
		pnlAuth.add(lblNominal, "cell 0 2");

		cmbNominal = new XJComboBox();
		cmbNominal.addItemListener(new ItemListener() {
			@Override
			public void itemStateChanged(ItemEvent e) {
				try {
					cmbNominalSelected();
		...
		pnlAuth.add(cmbNominal, "cell 1 2,growx");
		...
		panel.add(lblCreditStock, "cell 0 0");
		lblCreditStock
		...
		lblCreditStockValue.setFont(new Font("Tahoma", Font.BOLD,
				FONT_SIZE_SMALLEST));
		panel.add(lblCreditStockValue, "cell 1 0");
	...
	private void cmbNominalSelected() {
		ComboBoxObject comboBoxObject = (ComboBoxObject) cmbNominal
				.getSelectedItem();
		...
		try {
			...
			for (Voucher voucher : vouchers) {
				ComboBoxObject comboBoxObject = new ComboBoxObject(voucher,
						Formatter.formatNumberToString(voucher.getNominal()));
				comboBoxObjects.add(comboBoxObject);
			}
			cmbNominal.setModel(new DefaultComboBoxModel<ComboBoxObject>(
		...
		try {
			...
			List<SaleDetail> saleDetails = PrepaidSaleFacade.getInstance()
					.performSale(
							...
							voucher.getNominal(), item.getUnit(), session);
